...
namespace Cross_Sums
{
    ...
    {
        public OpenCell (int x, int y, CrossSumsMatrix matrix)
        {
            this.x = x;
            this.y = y;
            this.matrix = matrix;
   ...
   // If this list shrinks, we have moved closer to a solution.  The returnInfo does not reflect this.
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Up)
                matrix.MoveFocusUp(x, y);
            if (e.KeyCode == Keys.Down)
                matrix.MoveFocusDown(x,y);
            if (e.KeyCode == Keys.Right)
                matrix.MoveFocusRight(x, y);
            if (e.KeyCode == Keys.Left)
                matrix.MoveFocusLeft(x, y);
        }
        ...
        private void CreateCellForm(int x, int y)
        {
            ...
            Font = new Font(new FontFamily("Mistral"), 20);
